...
번호를 입력해 원하는 기능을 선택한다.

- **1. Create**: `이름`, `값`을 순서대로 입력하면 새 항목이 `id`와 함께 생성되어 `data.json`에 저장된다.
- **2. Read (전체 목록)**: 저장된 모든 항목을 출력한다. 데이터가 없으면 안내 메시지를 출력한다.
- **3. Read (검색)**: `id` 또는 `이름`을 입력하면 해당 항목을 찾아 출력한다. 숫자를 입력하면 id로, 그 외에는 이름으로 검색한다.
- **4. Update**: `id` 또는 `이름`으로 대상을 찾은 뒤, 수정할 필드명(`name` 또는 `value`)과 새 값을 입력하면 반영된다.
- **5. Delete**: `id` 또는 `이름`으로 대상을 찾은 뒤 `y`를 입력하면 삭제된다. `y`가 아닌 값을 입력하면 삭제가 취소된다.
- **0. 종료**: 프로그램을 종료한다.
